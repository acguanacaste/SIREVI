...
$sql="INSERT INTO `image` (anchura,altura,tipo,imagen) VALUES (".$info[0].",".$info[1].",'".$_FILES["userfile"]["type"]."','".$imagenEscapes."')";
$mysqli->query($sql);
...
<p>   <button class="btn waves-effect waves-light" type="submit" value="Guardar Imagen" name="action">Submit
...
<?php
$result=$mysqli->query("SELECT * FROM image ORDER BY id DESC");
if($result)
{
while($row=$result->fetch_array(MYSQLI_ASSOC))
{
echo "<img src='imagen_mostrar.php?id=".$row["id"]."' width='".$row["anchura"]."' height='".$row["abs(number)altura"]."'>";
}
}
?>
...
$result=$mysqli->query("SELECT * FROM `image` WHERE id=".$_GET["id"]);
