...
BEGIN
SELECT visitacion.provincia_id as ID, provincia.nombre AS Provincia, visitacion.nombre As Nombre,
...
DELIMITER $$
USE `sirevi`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `consulta_Total_Mensual_Sectores`(IN fechaStart DATE, IN fechaEnd DATE)
BEGIN
select sector.nombre, monthname(fecha),
...
from visitacion
inner join sector on visitacion.sector = sector.id
WHERE fecha BETWEEN fechaStart AND fechaEnd
group by sector.nombre
order by fecha ASC;
END$$
