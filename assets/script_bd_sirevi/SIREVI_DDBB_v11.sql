...
-- -----------------------------------------------------
-- Table `sirevi`.`listas_reproduccion`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `sirevi`.`listas_reproduccion` ;

CREATE TABLE IF NOT EXISTS `sirevi`.`listas_reproduccion` (
  `id` INT(4) NOT NULL,
...
-- -----------------------------------------------------
-- procedure ConsultaCampistas
-- -----------------------------------------------------

USE `sirevi`;
DROP procedure IF EXISTS `sirevi`.`ConsultaCampistas`;

DELIMITER $$
USE `sirevi`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ConsultaCampistas`()
BEGIN
SELECT visitacion.id, fecha, visitacion.proposito_visita, visitacion.subSector, sector.nombre, pais.nombre AS Pais, provincia.nombre AS provincia, noIdentificacion, visitacion.nombre
FROM visitacion
...
WHERE proposito_visita = 'Acampando';


/*SELECT distinct visitacion.sector, sector.nombre, visitacion.proposito_visita, visitacion.nombre,visitacion.subSector,
sum(nacional_adult+nacional_kid+extranjero_adult+extranjero_kid) AS Cant_Personas
 from visitacion inner join sector on visitacion.sector = sector.id
where visitacion.proposito_visita  = 'Acampando'
...
SELECT visitacion.provincia as ID, provincia.nombre Provincia, visitacion.nombre As Nombre,
 ...
 sum(visitacion.prepago) AS Prepagos, sum(visitacion.exonerado) AS Exonerados,
 
 sum(nacional_adult+nacional_kid+estudiantes+nacionales_exonerado+extranjero_adult+extranjero_kid+extranjero_exonerado+prepago) AS CantPersonas,
...
BEGIN
	/*Para hacer el reporte de SEMEC, logica en proceso...Cuando hay mas de un regitro en la fila, este me calcula solo 1 de ellos*/
	SELECT sector.nombre as AC, asp.tipo as Tipo_ASP, asp.nombre as ASP,
	...
	GROUP BY sector.nombre ORDER BY sector.id ASC;
